    ...
    if 2 < len(message.text) < 50:    
        ...
        if files:
            for file in files:
                ...
                filename = f"[{get_size(file.file_size)}] {file.file_name}"
                btn.append(
                    [InlineKeyboardButton(text=f"{filename}", url=f"https://jdisk.sktamilserialbots.com/st?api=d22703d9fbde338f22ef64fa3f6a8b97d60b0d18&url=https://telegram.dog/{nyva}?start=tamilroars{file_id}")]
                )
        else:
            return
        if not btn:
